		...
		if (typeof name === 'string') {
			if (value === undefined) {
				return elem.currentStyle ?
					...
					(document.defaultView?document.defaultView.getComputedStyle(elem, false)[name]:'');
			} else {
...
(function(){
	...
	$.doc = function(win){
		...
		//yanghl 2010-12-15 add
		//为了控制弹出的窗口大小的时候出现滚动条，现在高度是50
		...
		return {
			...
			height: Math.max(cheight, dom.scrollHeight),	// 页面长度
				  ...
				  <td <% if (icon) { %>colspan="2"<% } %> class="aui_td_title">\
					 <div class="aui_title_wrap">\
						<div id="<%=id%>title" class="aui_title">\
					  	...
					  	<a id="<%=id%>close" class="aui_close" href="#" onfocus="this.blur();"><%=closeText%></a>\
					 </div>\
				  </td>\
				</tr>\
				  <% } %>\
				...
				<tr>\
				  <td <% if (icon) { %>colspan="2"<% } %> class="aui_td_buttons">\
					  <div class="aui_buttons_wrap">\
						<% if (yesFn) { %><button class="aui_button_pro" id="<%=id%>yes"><%=yesText%></button><% } %>\
						<% if (noFn) { %><button class="aui_button_pro" id="<%=id%>no"><%=noText%></button><% } %>\
					  </div>\
					</td>\
				</tr>\
				<% } %>\
...
var dialog = function(data){
	...
	//modify huanghui@021-7-10  当$('body')[0]为空时，append方法中报错，其他地方也有未定义的错误。
	var _body = $('body')[0];
	if(_body==null ||_body=='' ||_body=='undefined'){
		return;
	}
	$('body').append(html);
...
// 针对移动设备对fixed支持不完整可能带来遮罩无法全部覆盖的问题
	...
	var api = {
		...
		// 关闭对话框
		close: function() {
			//modify huanghui@2012-12-21 延迟执行close方法，确保点击叉叉的时候window.onbeforeunload里面的方法可以执行
			//ie浏览器下会有将iframe的src清空加入一个false，实际上已经进行了延迟，不需要再延迟
		...
		// 关闭按钮行为  closeFn 第一次进入closeFn
		closeFn: function(event){
			event && $.stopDefault(event);
			var fn =  data.closeFn || data.noFn;
			//modify huanghui@2012-12-27 关闭art的叉叉时会进入到该方法，直接执行api.close与页面中调用的art.dialog.close()方法相同。
			//return typeof fn !== 'function' || fn.call(api, window) !== false ? api.close() : api;
			return api.close.call();
		},
	...
	if ($.isIE6) {
		...
		$.each(list, function(i, elem){
			...
			if (png) {
				...
				elem.style.cssText = "font-size:12px; line-height:12px; text-indent:0em;";
...
var defaults = {
	...
	show: true,					// 是否显示
	zIndex: 100005,				// 对话框最低叠加高度值(重要：此值不能过高，否则会导致Opera、Chrome等浏览器表现异常) [*]
 ...
 if (!('ontouchend' in document)) {
	...
	var winResize = function(){
		...
		if (Math.abs(o.height - docH.height) > 0 || 
			Math.abs(o.width - docH.width) === 17) {
...
$.fn.dialog.inner = function(win, fn){
	...
	if (win.document.getElementsByTagName('frameset').length !== 0)
        ...
        //@zhangwq,2012-8-10,递归调用
        //在frameset上Uncaught RangeError: Maximum call stack size exceeded;
...
$.fn.dialog.open = function(url, options){
	var load, $iframe, iwin,
		opt = options,
		id = _name + 'Open',
		//yanghl add 为了防止一个页面打开多层的artDialog对话框。
		//isWindowTop = $.dialog.defaults.window?$.dialog.defaults.window:'top',
		jumpFlag = url.indexOf("/common/djframe.htm")>-1;
		var temlUrl = jumpFlag?url:basePath+"/toArtDialog.do";
		data = {
			window: 'top',
			content: {url: temlUrl},
			...
			initFn: function(here){
				...
				if(!jumpFlag){
					...
					var iwin1 = $iframe[0];
					if (iwin.attachEvent){
					    iwin.attachEvent("onload", function(){
					    	iwin.document.getElementById("form1").action = realUrl;
					    	if(param!=null&&param!=""){
					    		...
					    		for(var i=0;i<params.length;i++){
					    			...
					    			paramhtml += "<input type='hidden' name='"+paramName+"' value='"+paramValue+"' />";
					    		}
					    		iwin.document.getElementById("param").innerHTML  = paramhtml;
					    	}
				    		iwin.document.getElementById("form1").submit();
					    });
					} else {
					    iwin1.onload = function(){
					    	iwin.document.getElementById("form1").action = realUrl;
					    	if(param!=null&&param!=""){
					    		var params = param.split("&");
					    		var paramhtml = "";
				...
				load = function(){
					...
					$.dialog.inner(iwin, function(){
						...
						var iwinArt  = iwin.art || '';
						if(iwinArt !=null && iwinArt !='' && iwinArt!='undefined'){
							iwinArt.fn.dialog.close = function(){
								api.close&&api.close();
							};
							//lxj添加返回值函数
							iwinArt.fn.dialog.sendMsg = function(params){								
								api.data.sendMsg&&api.data.sendMsg(params);
							};
							// 传递来源window对象
							iwinArt.fn.dialog.parent = window;
						}
					});
...
//yanghl 2010-12-12 add 为了防止打开的页面在最顶层显示
var panelWordids="";
$.fn.dialog.open = function(url, options,isTop){
	if(url.indexOf("panelWordids")>-1){
		panelWordids=url.substring(url.indexOf("panelWordids"),url.indexOf("-------"));
	//	url=url.split(url.substring(url.indexOf("panelWordids")-1,url.length))[0];
	}
	var load, $iframe, iwin,
		opt = options,
		id = _name + 'Open',
		...
		data = {
			...
			initFn: function(here){
				...
				if(!jumpFlag){
					...
					var iwin1 = $iframe[0];
	                iwin1.setAttribute("data-xUrl",url);//方便调试,在iframe中添加属性,存放真实的链接@zhng,2012,5-14
					if (iwin.attachEvent){
					    iwin.attachEvent("onload", function(){
					    	iwin.document.getElementById("form1").action = realUrl;
					    	if(param!=null&&param!=""){
...
//					    	msj 修改，参数直接在action里，不改成隐藏框
					    	//iwin.document.getElementById("form1").action = url;
				    		iwin.document.getElementById("form1").submit();
					    });
					} else {
					    iwin1.onload = function(){
//					    	iwin.document.getElementById("form1").action = realUrl+"?tokenid="+tokenID;
//					    	if(param!=null&&param!=""){
//					    		var params = param.split("&");
//					    		var paramhtml = "";
//					    		for(var i=0;i<params.length;i++){
...
//					    	msj 修改，参数直接在action里，不改成隐藏框
					    	iwin.document.getElementById("form1").action = url;
	                        //重置<==safari,chrome dev下弹出窗口内容溢出时会造成页面闪烁不停;@zhng,2012-4-18
	                        iwin1.onload = null;
				    		iwin.document.getElementById("form1").submit();
					    };
					}
				...
				load = function(){
					...
					$.dialog.inner(iwin, function(){
						...
						if(iwinArt !=null && iwinArt !='' && iwinArt!='undefined'){
							...
							//limj 2011-12-20 重置artDialog窗口大小
							iwinArt.fn.dialog.size = function(weight,height){
								api.size(weight,height);
							};
							//lxj添加返回值函数
							iwinArt.fn.dialog.sendMsg = function(params){								
								api.data.sendMsg&&api.data.sendMsg(params);
			...
			closeFn: function(here){
				if(panelWordids!=""){
					var tempids=panelWordids.split("=")[1];
					var realtempids=tempids.split(",");
					for (var i = 0; i < realtempids.length; i++) {
						if(document.getElementById(realtempids[i].split("_")[0]+"_TANGER_OCX")!=null&&document.getElementById(realtempids[i].split("_")[0]+"_TANGER_OCX")!=undefined&&document.getElementById(realtempids[i].split("_")[0]+"_TANGER_OCX")!=""){
							$("#"+realtempids[i]).css("display","block");
						}
					}
				}
...
//-------------end
/**
 *更改:ipad支持;使用标准方式检测客户端支持特性方式
 *@更改行数:893;975;978;1346;1376;1366;1666;
 */