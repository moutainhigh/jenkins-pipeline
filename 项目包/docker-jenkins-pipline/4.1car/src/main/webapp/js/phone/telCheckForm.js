$(function() {
	...
	$('input:radio[name="otherLoanMark"]').change(function() {	
		...
		showOrHideBorrowerInfo(p1,'otherloan');
	});
	// 页签切换效果
...
function saveOneContact(t){
	...
	if(checkForm(form)){
		...
		$.ajax({
			...
			url : ctx + "/verify/telcheck/saveCallContactInfo",
			beforeSend: function(XMLHttpRequest){
            	waitingDialog.show("呼叫中...");
            },
            complete: function(XMLHttpRequest, textStatus){
			...
			success : function(data) {
				if( data != null){
					if(data.id != "" && data.id != null){
						// 电话只读
						form.find("input[name='telNum']").attr("readonly","readonly");
						// 来源只读
						form.find("select[name='dictTelSource']").attr("disabled","disabled");
						
						// 隐藏删除按钮
						form.find("tr").eq(0).find("td").eq(0).find("a[name='aID']").attr("class","hide");
...
function submitContactInfoWithAll(t) {
	...
	var dataParam = $(window.parent.document).find("form[id='param']").serialize();
	// 拼接电话录音参数
	var param = makeDataWithAll(t);
	$.ajax({
		type : "POST",
		data : form.serialize()+encodeURI(param)+"&" + dataParam,
		url : ctx + "/verify/telcheck/saveContactInfo",
		success : function(data) {
			form.find("selected[name='dictCertType']").attr("disabled","disabled");
			// 如果是新添加的联系人信息赋ID，置灰，显示拨打电话按钮
			if (id == null || id == "") {
				form.find("input[name='id']").val(data);
			}
			// 显示拨打图标
			form.find("input[name='telNum']").next("a").attr("class","");
			// 设置电话为readonly
			form.find("input[name='telNum']").attr("readonly","readonly");
			// 设置来源为disabled
			form.find("select[name='dictTelSource']").attr("disabled","disabled");
...
function relationChange(tt){
	...
	var hasMate = form.find("input[name='hasMate']").val();
	if(hasMate == '1'){
		if(relation==2){
			var mateName = form.find("input[name='mateName']").val();
			var mateCertType = form.find("input[name='mateCertType']").val();
			var mateCertNum = form.find("input[name='mateCertNum']").val();
			
			form.find("input[name='name']").val(mateName);
			form.find("input[name='name']").attr("readonly","readonly");
			
			form.find("select[name='dictCertType']").val(mateCertType);
			form.find("select[name='dictCertType']").attr("readonly","readonly");
			
			form.find("input[name='customerCertNum']").val(mateCertNum);
			form.find("input[name='customerCertNum']").attr("readonly","readonly");
			
			$(tt).parents("tr:first").nextAll("tr:first").show();
		}else{
			$(tt).parents("tr:first").nextAll("tr:first").hide();
			form.find("input[name='name']").removeAttr("readonly");
		}
	}else{
		if(relation == 2){
...
function addPersonalNums(t,type){	
	if(type=='1'){
		...
		html.find("font[id='phone']").html("手机号码");
		...
		html.find("font[id='phone']").html("家庭固话");
		html.find("a[id='jia']").hide();
		html.attr("name","jtgh");
...
function delContact(t,id){
	$.ajax({
		url : ctx + "/verify/telcheck/delContact",
		data : {"id":id},
		type : "POST",
		dataType:'json',
		success : function(data) {
...
function zcContact(t,id){
	...
	form.find("select[name='dictTelSource']").removeAttr("disabled");
	
	//执行暂存操作
	$.ajax({
		type : "POST",
		data : form.serialize()+"&"+dataParam,
		url : ctx + "/verify/telcheck/zcContact",
		success : function(data) {
			if (data != null && data != "") {
				var getId = data.split(",")[0];
				var chachong = data.split(",")[1];
				var phoneNumber = form.find("input[name='telNum']").val();
