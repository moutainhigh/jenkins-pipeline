...
$(function() {
	...
	$("#saveCarLoanRecheckInfo").click(function() {
		...
		$.ajax({
			...
			url: ctx + "/carloan/carLoanCheckInfo/" + ((checkType == '10' || checkType == '11' || checkType == '48') ? "asynCarLoanReCheckHandle" : "asynCarLoanFinalCheckHandle"),
	...
	$('#abandonSave').click(function() {
		...
		$.ajax({
			...
			url: ctx + "/carloan/carLoanCheckInfo/" + ((checkType == '10') ? "asynCarLoanReCheckHandle" : "asynCarLoanFinalCheckHandle"),
			success : function(data) {
				if (data == "true") {
					$('#modal_Abandon').modal('hide');
					var win = top.window;
