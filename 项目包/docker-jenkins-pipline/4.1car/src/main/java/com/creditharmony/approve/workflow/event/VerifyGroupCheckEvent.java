 ...
 */
@Service("Verify_GroupCheck_Flow")
public class VerifyGroupCheckEvent extends ApproveCommonService implements ExEvent{
	...
	public void invoke(WorkItemView workItem) {
		...
		if(response.equals(ApproveRouteConstants.GROUPCHECK_TO_INTERESTRATE)){ // 利率审核
			...
			flowView.setCheckResult(CheckResult.XS_THRED_PASS.getCode());
		...
		}else if(response.equals(ApproveRouteConstants.GROUPCHECK_TO_FINALCHECK)){ // 大于等于30万，终审组提交到终审
			flowView.setLoanApplyStatus(LoanApplyStatus.GROUP_CHECK_PASS_FINAL_CHECK.getName());
			flowView.setLoanStatusCode(LoanApplyStatus.GROUP_CHECK_PASS_FINAL_CHECK.getCode());
			flowView.setCheckResult(CheckResult.XS_THRED_SUBMIT.getCode());
			flowView.setStepName(step);
			super.fromCheck(flowView);
			toNull(flowView);
		}else if(response.equals(ApproveRouteConstants.GROUPCHECK_TO_END)){ //拒借 GROUP_CHECK_REJECT
			flowView.setLoanApplyStatus(LoanApplyStatus.GROUP_CHECK_REJECT.getName());
			flowView.setLoanStatusCode(LoanApplyStatus.GROUP_CHECK_REJECT.getCode());
			flowView.setCheckResult(CheckResult.XS_THRED_REFUSAL.getCode());
			flowView.setFlag(true);
			flowView.setStepName(step);
			flowView.setRefuseReason(getReason(flowView));
			String id = super.fromCheck(flowView);
			finalUpdate(id,flowView.getLoanCode());
		}else if(response.equals(ApproveRouteConstants.CHECK_TO_PREVIOUS)){ // 终审组退回协商到复审
			flowView.setLoanApplyStatus(LoanApplyStatus.GROUP_CHECK_COUSULT_RECHECK.getName());
			flowView.setLoanStatusCode(LoanApplyStatus.GROUP_CHECK_COUSULT_RECHECK.getCode());
			flowView.setCheckResult(CheckResult.XS_THRED_BACK.getCode());
			flowView.setStepName(step);
