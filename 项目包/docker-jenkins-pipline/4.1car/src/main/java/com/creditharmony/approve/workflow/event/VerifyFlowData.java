 ...
 */
@Service("Verify_Flow_Data")
public class VerifyFlowData extends BaseService implements BusinessLoadCallBack {
	@Autowired 
	...
	public BaseBusinessView load(String applyId, String stepName) {
		...
		res=loanCustomerDao.getInfoForVerify(applyId);
		res.setCheckType(ApproveCheckType.XS_APPROVE_CHECK_TYPE.getCode());
		...
		String queryDate  = DateUtils.formatDate(res.getTimeOutPoint(), "yyyyMMdd");
		DiskInfoView diskInfoView = diskInfoService.findDiskName(queryDate, res.getDictSourceType());
