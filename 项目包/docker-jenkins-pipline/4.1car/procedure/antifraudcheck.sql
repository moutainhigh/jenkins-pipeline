DROP FUNCTION jk.antifraudcheck(character varying);
/**
 * 反欺诈判定(查重、销售人员、黑灰名单、欺诈案件)
 * @修改履历  V1.0 2016/2/20 创建
 *        V2.0 
 */
CREATE OR REPLACE FUNCTION jk.antifraudcheck(IN loancode character varying, OUT outvalue character varying)
RETURNS character varying AS
$BODY$
DECLARE BACKVALUE VARCHAR(32);--返回值
		T_COUNT INT;--规则统计
		SUM_COUNT INT;--所有统计
		LOANSTAUTS VARCHAR (50); --借款状态
		HIS_ID VARCHAR(32);   --历史表ID
		MSG VARCHAR (5000);  -- 反欺诈说明信息
		REPORT_ID VARCHAR(32);--反欺诈提报ID
		JUDGE_ID VARCHAR(32); --反欺诈判定ID
		JUDGE_RESULT VARCHAR(32); --判定结果
BEGIN
	JUDGE_RESULT := '未触发';
	BACKVALUE:='0';
	SUM_COUNT:=0;
...
SELECT A.LOAN_CODE N_LOAN_CODE,COALESCE(A.CUSTOMER_PHONE_FIRST,'') N_PHONE_ONE,COALESCE(A.CUSTOMER_PHONE_SECOND,'') N_PHONE_TWO,
       ...
       M1.AREA_NAME||M2.AREA_NAME||M3.AREA_NAME||B.CUSTOMER_REGISTER_ADDRESS AS OLD_HOME_ADDERSS,
       M4.AREA_NAME||M5.AREA_NAME||M6.AREA_NAME||CO.COMP_ADDRESS AS OLD_COMP_ADDERSS
FROM JK.T_JK_LOAN_CUSTOMER A INNER JOIN JK.T_JK_LOAN_CUSTOMER B ON A.LOAN_CODE=loancode AND A.CUSTOMER_CERT_NUM=B.CUSTOMER_CERT_NUM AND A.LOAN_CODE<>B.LOAN_CODE
							 INNER JOIN JK.T_JK_LOAN_INFO TT ON B.LOAN_CODE=TT.LOAN_CODE AND TT.OUTTO_LOAN_TIME IS NOT NULL                             
                              ...
                              LEFT JOIN JK.T_GL_PROVINCE_CITY M1 ON B.CUSTOMER_REGISTER_PROVINCE = M1.AREA_CODE AND M1.AREA_TYPE = '1'
							  LEFT JOIN JK.T_GL_PROVINCE_CITY M2 ON B.CUSTOMER_REGISTER_CITY = M2.AREA_CODE AND M2.AREA_TYPE = '2'
                              LEFT JOIN JK.T_GL_PROVINCE_CITY M3 ON B.CUSTOMER_REGISTER_AREA = M3.AREA_CODE AND M3.AREA_TYPE = '3'
                              LEFT JOIN JK.T_GL_PROVINCE_CITY M4 ON CO.COMP_PROVINCE = M4.AREA_CODE AND M4.AREA_TYPE = '1'
							  LEFT JOIN JK.T_GL_PROVINCE_CITY M5 ON CO.COMP_CITY = M5.AREA_CODE AND M5.AREA_TYPE = '2'
                              LEFT JOIN JK.T_GL_PROVINCE_CITY M6 ON CO.COMP_ARER = M6.AREA_CODE AND M6.AREA_TYPE = '3'
)
INSERT INTO JK.T_JK_ANTIFRAUD_REPEAT(ID,LOAN_CODE_NOW,RULES_CODE,LOAN_CODE,CUSTOMER_NAME,REPEAT_CERT_NUM,REPEAT_INTO_TIME,REPEAT_RESULT, 
                                 REPEAT_RESULT_MSG,REPEAT_RELATION,REPEAT_NAME_DIFF_FLAG,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),N_LOAN_CODE,'C001',O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_CERT_NUM,CUSTOMER_INTO_TIME,OPERATE_RESULT,'户籍地址:'||OLD_HOME_ADDERSS,'本人','0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_C_RULE
WHERE N_HOMEPLACE!=O_HOMEPLACE
GROUP BY N_LOAN_CODE,O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_INTO_TIME,CUSTOMER_CERT_NUM,OLD_HOME_ADDERSS,OPERATE_RESULT
UNION ALL
...
UNION ALL
SELECT GETUUID32(),N_LOAN_CODE,'C006',O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_CERT_NUM,CUSTOMER_INTO_TIME,OPERATE_RESULT,'单位地址:'||OLD_COMP_ADDERSS,'本人','0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_C_RULE 
WHERE DATE_DIFF<180 AND N_COMP_ADDRESS!=O_COMP_ADDRESS AND N_COMP_NAME=O_COMP_NAME AND N_COMP_TEL=O_COMP_TEL
GROUP BY N_LOAN_CODE,O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_INTO_TIME,CUSTOMER_CERT_NUM,OLD_COMP_ADDERSS,OPERATE_RESULT
UNION ALL
...
FROM TEMP_C_RULE 
WHERE (N_PRODUCT_TYPE='A005' OR N_PRODUCT_TYPE='A006') AND N_COMP_ADDRESS!=N_LIVE_ADDRESS AND N_COMP_NAME=O_COMP_NAME AND O_COMP_ADDRESS=O_LIVE_ADDRESS
...
FROM TEMP_C_RULE 
WHERE (N_PRODUCT_TYPE='A005' OR N_PRODUCT_TYPE='A006') AND O_COMP_ADDRESS!=O_LIVE_ADDRESS AND N_COMP_NAME=O_COMP_NAME AND N_COMP_ADDRESS=N_LIVE_ADDRESS
GROUP BY N_LOAN_CODE,O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_INTO_TIME,CUSTOMER_CERT_NUM,OPERATE_RESULT
UNION ALL
...
FROM TEMP_C_RULE 
WHERE (N_PRODUCT_TYPE='A005' OR N_PRODUCT_TYPE='A006') AND N_COMP_NAME!=O_COMP_NAME AND N_COMP_ADDRESS=N_LIVE_ADDRESS AND O_COMP_ADDRESS=O_LIVE_ADDRESS
GROUP BY N_LOAN_CODE,O_LOAN_CODE,O_CUSTOMER_NAME,CUSTOMER_INTO_TIME,CUSTOMER_CERT_NUM,OPERATE_RESULT;
---R系列规则
WITH TEMP_R_RULE AS (
...
SELECT A.LOAN_CODE,A.CERT_NUM,A.PHONE_ONE,A.PHONE_TWO,A.CUSTOMER_NAME,A.N_CUSTOMTER_ID,B.COMP_NAME,C.CONTACT_NAME,
       COALESCE(C.CONTACT_MOBILE,'') CONTACT_MOBILE,C.RELATION_TYPE,ROW_NUMBER() OVER(PARTITION BY A.LOAN_CODE,C.ID ORDER BY B.CREATE_TIME DESC) N_COMPANY_NO
FROM TEMP_R_RULE A LEFT JOIN JK.T_JK_LOAN_COMPANY B ON A.LOAN_CODE=B.LOAN_CODE AND A.N_CUSTOMTER_ID=B.R_ID AND COALESCE(B.COMP_NAME,'')!=''
                   LEFT JOIN JK.T_JK_CONTACT C ON A.LOAN_CODE=C.LOAN_CODE AND A.N_CUSTOMTER_ID=C.R_CUSTOMER_COBORROWER_ID
),TEMP_R001 AS (
SELECT A.LOAN_CODE N_LOAN_CODE,B.LOAN_CODE O_LOAN_CODE,B.CUSTOMER_NAME,B.CUSTOMER_CERT_NUM,
...
FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_CUSTOMER B ON ((A.PHONE_ONE!='' AND (A.PHONE_ONE=B.CUSTOMER_PHONE_FIRST OR A.PHONE_ONE=B.CUSTOMER_PHONE_SECOND)) 
                   ...
                   INNER JOIN JK.T_JK_LOAN_INFO C ON B.LOAN_CODE=C.LOAN_CODE AND C.OUTTO_LOAN_TIME IS NOT NULL
                   LEFT JOIN JK.T_JK_AUDIT_RECORD D ON C.AUDIT_ID=D.ID
WHERE A.CUSTOMER_NAME=B.CUSTOMER_NAME
ORDER BY B.CREATE_TIME DESC LIMIT 10
),TEMP_R002 AS (
SELECT A.LOAN_CODE N_LOAN_CODE,B.LOAN_CODE O_LOAN_CODE,B.CUSTOMER_NAME,B.CUSTOMER_CERT_NUM,
       C.CUSTOMER_INTO_TIME,D.OPERATE_RESULT,CASE WHEN A.CUSTOMER_NAME!=B.CUSTOMER_NAME THEN B.CUSTOMER_NAME ELSE '' END RESULT_MSG
FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_CUSTOMER B ON ((A.PHONE_ONE!='' AND (A.PHONE_ONE=B.CUSTOMER_PHONE_FIRST OR A.PHONE_ONE=B.CUSTOMER_PHONE_SECOND)) 
...
FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_MATE B ON ((A.PHONE_ONE!='' AND A.PHONE_ONE=B.MATE_TEL) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.MATE_TEL)) 
                   ...
                   INNER JOIN JK.T_JK_LOAN_INFO D ON C.LOAN_CODE=D.LOAN_CODE AND D.OUTTO_LOAN_TIME IS NOT NULL
...
WHERE (SELECT COUNT(1) FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_MATE B ON ((A.PHONE_ONE!='' AND A.PHONE_ONE=B.MATE_TEL) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.MATE_TEL)) 
                                AND A.CUSTOMER_NAME!=B.MATE_NAME AND A.CERT_NUM!=B.MATE_CERT_NUM AND A.LOAN_CODE!=B.LOAN_CODE
                         INNER JOIN JK.T_JK_LOAN_CUSTOMER C ON B.LOAN_CODE=C.LOAN_CODE AND A.CERT_NUM!=C.CUSTOMER_CERT_NUM
                         INNER JOIN JK.T_JK_LOAN_INFO D ON C.LOAN_CODE=D.LOAN_CODE AND D.OUTTO_LOAN_TIME IS NOT NULL)>=2
ORDER BY B.CREATE_TIME DESC LIMIT 10
),TEMP_R003_R013 AS (
SELECT A_ROW_NO.* FROM
(SELECT A.*,ROW_NUMBER() OVER(PARTITION BY N_LOAN_CODE,RULES_CODE ORDER BY O_LOAN_CODE DESC) ROW_NO FROM
...
FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_MATE B ON (((A.PHONE_ONE!='' AND A.PHONE_ONE=B.MATE_TEL) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.MATE_TEL))
                         ...
                         INNER JOIN JK.T_JK_LOAN_CUSTOMER C ON B.LOAN_CODE=C.LOAN_CODE AND A.CERT_NUM<>C.CUSTOMER_CERT_NUM
                         INNER JOIN JK.T_JK_LOAN_INFO D ON C.LOAN_CODE=D.LOAN_CODE AND D.OUTTO_LOAN_TIME IS NOT NULL
                          LEFT JOIN JK.T_JK_AUDIT_RECORD E ON D.AUDIT_ID=E.ID) A
WHERE RULES_CODE!='') A_ROW_NO
WHERE ROW_NO<=10
),TEMP_R014_R023 AS (
SELECT A_ROW_NO.* FROM
(SELECT A.*,ROW_NUMBER() OVER(PARTITION BY N_LOAN_CODE,RULES_CODE ORDER BY O_LOAN_CODE DESC) ROW_NO FROM
(SELECT A.N_LOAN_CODE,A.O_LOAN_CODE,A.O_CUSTOMER_NAME CUSTOMER_NAME,A.CUSTOMER_CERT_NUM,A.CUSTOMER_INTO_TIME,A.O_CONTACT_NAME CONTACT_NAME,A.O_COMP_NAME COMP_NAME,A.OPERATE_RESULT,
	CASE WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND (A.PHONE_ONE=A.O_CONTACT_MOBILE OR A.PHONE_TWO=A.O_CONTACT_MOBILE) 
		  AND A.O_RELATION_TYPE='0' AND A.N_CUSTOMER_NAME!=A.O_CONTACT_NAME THEN 'R014'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND (A.PHONE_ONE=A.O_CONTACT_MOBILE OR A.PHONE_TWO=A.O_CONTACT_MOBILE)
		  AND A.O_RELATION_TYPE='1' AND A.N_CUSTOMER_NAME!=A.O_CONTACT_NAME THEN 'R015'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND (A.PHONE_ONE=A.O_CONTACT_MOBILE OR A.PHONE_TWO=A.O_CONTACT_MOBILE)
		  AND A.O_RELATION_TYPE='2' AND A.N_CUSTOMER_NAME!=A.O_CONTACT_NAME THEN 'R016'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='0' AND A.O_RELATION_TYPE='0' 
		  AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME THEN 'R017'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='0' AND A.O_RELATION_TYPE='1' 
		  AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME THEN 'R018'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='0' AND A.O_RELATION_TYPE='2' 
		  AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME THEN 'R019'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='1' AND A.O_RELATION_TYPE='0' 
		  AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME THEN 'R020'
	     WHEN A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='1' AND A.O_RELATION_TYPE='1' AND A.N_CONTACT_NAME=A.O_CONTACT_NAME AND A.N_COMP_NAME!=A.O_COMP_NAME THEN 'R021'
	     WHEN A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='1' AND A.O_RELATION_TYPE='1' AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME AND A.N_COMP_NAME!=A.O_COMP_NAME THEN 'R022'
	     WHEN A.N_COMPANY_NO=1 AND A.O_COMPANY_NO=1 AND A.N_CONTACT_MOBILE=A.O_CONTACT_MOBILE AND A.N_RELATION_TYPE='1' AND A.O_RELATION_TYPE='2' 
		  AND A.N_CONTACT_NAME!=A.O_CONTACT_NAME THEN 'R023'
	ELSE '' END RULES_CODE
FROM (SELECT A.LOAN_CODE N_LOAN_CODE,B.LOAN_CODE O_LOAN_CODE,C.CUSTOMER_CERT_NUM,D.CUSTOMER_INTO_TIME,F.OPERATE_RESULT,A.COMP_NAME N_COMP_NAME,A.N_COMPANY_NO,
       A.PHONE_ONE,A.PHONE_TWO,A.CUSTOMER_NAME N_CUSTOMER_NAME,A.RELATION_TYPE N_RELATION_TYPE,A.CONTACT_MOBILE N_CONTACT_MOBILE,A.CONTACT_NAME N_CONTACT_NAME,
       B.CONTACT_MOBILE O_CONTACT_MOBILE,B.RELATION_TYPE O_RELATION_TYPE,B.CONTACT_NAME O_CONTACT_NAME,C.CUSTOMER_NAME O_CUSTOMER_NAME,E.COMP_NAME O_COMP_NAME,
       ROW_NUMBER() OVER(PARTITION BY B.LOAN_CODE,B.ID ORDER BY B.CREATE_TIME DESC) O_COMPANY_NO
      FROM TEMP_CONTACT A INNER JOIN JK.T_JK_CONTACT B ON ((A.PHONE_ONE!='' AND A.PHONE_ONE=B.CONTACT_MOBILE) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.CONTACT_MOBILE) OR (A.CONTACT_MOBILE!='' AND A.CONTACT_MOBILE=B.CONTACT_MOBILE)) AND A.LOAN_CODE!=B.LOAN_CODE
                    ...
                    INNER JOIN JK.T_JK_LOAN_INFO D ON C.LOAN_CODE=D.LOAN_CODE AND D.OUTTO_LOAN_TIME IS NOT NULL
                     ...
                     LEFT JOIN JK.T_JK_AUDIT_RECORD F ON D.AUDIT_ID=F.ID) A) A
WHERE RULES_CODE!='') A_ROW_NO
WHERE ROW_NO<=10
),TEMP_R024_R026 AS (
SELECT A_ROW_NO.* FROM
...
(SELECT A.LOAN_CODE N_LOAN_CODE,C.LOAN_CODE O_LOAN_CODE,D.CUSTOMER_NAME,D.CUSTOMER_CERT_NUM,E.CUSTOMER_INTO_TIME,C.COMP_TEL,C.COMP_NAME,F.OPERATE_RESULT,
       CASE WHEN B.COMP_NAME=C.COMP_NAME AND B.COMP_TEL=C.COMP_TEL AND COALESCE(DATE_PART('DAY',A.CUSTOMER_INTO_TIME - E.CUSTOMER_INTO_TIME),0)<60 THEN 'R024'
	    WHEN B.COMP_NAME=C.COMP_NAME AND B.COMP_TEL!=C.COMP_TEL AND COALESCE(DATE_PART('DAY',A.CUSTOMER_INTO_TIME - E.CUSTOMER_INTO_TIME),0)<90 THEN 'R025'
	    WHEN B.COMP_NAME!=C.COMP_NAME AND B.COMP_TEL=C.COMP_TEL AND COALESCE(DATE_PART('DAY',A.CUSTOMER_INTO_TIME - E.CUSTOMER_INTO_TIME),0)<90 THEN 'R026'
       ELSE '' END RULES_CODE
FROM TEMP_R_RULE A INNER JOIN JK.T_JK_LOAN_COMPANY B ON A.LOAN_CODE=B.LOAN_CODE AND A.N_CUSTOMTER_ID=B.R_ID
                         ...
                         INNER JOIN JK.T_JK_LOAN_INFO E ON C.LOAN_CODE=E.LOAN_CODE AND E.OUTTO_LOAN_TIME IS NOT NULL
                          LEFT JOIN JK.T_JK_AUDIT_RECORD F ON E.AUDIT_ID=F.ID) A
WHERE RULES_CODE!='') A_ROW_NO
WHERE ROW_NO<=10
)
INSERT INTO JK.T_JK_ANTIFRAUD_REPEAT(ID,LOAN_CODE_NOW,RULES_CODE,LOAN_CODE,CUSTOMER_NAME,REPEAT_CERT_NUM,REPEAT_INTO_TIME,REPEAT_RESULT, 
...
SELECT GETUUID32(),N_LOAN_CODE,'R001',O_LOAN_CODE,CUSTOMER_NAME,CUSTOMER_CERT_NUM,CUSTOMER_INTO_TIME,OPERATE_RESULT,RESULT_MSG,'本人' REPEAT_RELATION,'0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_R001
UNION ALL
SELECT GETUUID32(),N_LOAN_CODE,'R002',O_LOAN_CODE,CUSTOMER_NAME,CUSTOMER_CERT_NUM,CUSTOMER_INTO_TIME,OPERATE_RESULT,RESULT_MSG,'本人' REPEAT_RELATION,'0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_R002
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R014' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R014')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R015' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R015')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R016' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R016')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R017' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R017')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R018' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R018')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R019' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R019')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R020' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R020')>=2
UNION ALL
...
FROM TEMP_R014_R023
WHERE RULES_CODE='R023' AND (SELECT COUNT(1) FROM TEMP_R014_R023 WHERE RULES_CODE='R023')>=2
UNION ALL
...
FROM TEMP_R024_R026
WHERE RULES_CODE='R024' AND (SELECT COUNT(1) FROM TEMP_R024_R026 WHERE RULES_CODE='R024')>=4
UNION ALL
...
FROM TEMP_R024_R026
WHERE RULES_CODE='R025' AND (SELECT COUNT(1) FROM TEMP_R024_R026 WHERE RULES_CODE='R025')>=3
UNION ALL
...
FROM TEMP_R024_R026
WHERE RULES_CODE='R026' AND (SELECT COUNT(1) FROM TEMP_R024_R026 WHERE RULES_CODE='R026')>=3;
--查重触犯规则主表插入
INSERT INTO JK.T_JK_ANTIFRAUD_OFFEND_INFO(ID,LOAN_CODE,RULES_CODE,DICT_OFFEND_TYPE,OFFEND_RELIEVE_STATUS,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),LOAN_CODE_NOW,RULES_CODE,'3','0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM JK.T_JK_ANTIFRAUD_REPEAT 
WHERE LOAN_CODE_NOW=loancode
...
INSERT INTO JK.T_JK_ANTIFRAUD_OFFEND_SALES(ID,LOAN_CODE_NOW,RULES_CODE,OFFEND_SALES_NAME,OFFEND_TEL,OFFEND_NAME,WORK_FLAG,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),A.LOAN_CODE,'S001',B.SALES_NAME,B.SALES_TEL,'本人('||A.CUSTOMER_NAME||')',B.WORK_FLAG,'BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_SALES A INNER JOIN JK.T_JK_ANTIFRAUD_SALES_STAFF B ON (A.PHONE_ONE!='' AND A.PHONE_ONE=B.SALES_TEL) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.SALES_TEL)
GROUP BY A.LOAN_CODE,B.SALES_NAME,B.SALES_TEL,B.WORK_FLAG,A.CUSTOMER_NAME,A.PHONE_ONE,A.PHONE_TWO
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'S002',B.SALES_NAME,B.SALES_TEL,'配偶('||A.MATE_NAME||')',B.WORK_FLAG,'BATCH',SYSDATE,'BATCH',SYSDATE
FROM TEMP_SALES A INNER JOIN JK.T_JK_ANTIFRAUD_SALES_STAFF B ON (A.MATE_TEL!='' AND A.MATE_TEL=B.SALES_TEL)
GROUP BY A.LOAN_CODE,B.SALES_NAME,B.SALES_TEL,B.WORK_FLAG,A.MATE_NAME,A.MATE_TEL
UNION ALL
...
FROM TEMP_SALES A INNER JOIN JK.T_JK_ANTIFRAUD_SALES_STAFF B ON (A.CONTACT_MOBILE!='' AND A.CONTACT_MOBILE=B.SALES_TEL);
--销售人员主表插入
INSERT INTO JK.T_JK_ANTIFRAUD_OFFEND_INFO(ID,LOAN_CODE,RULES_CODE,DICT_OFFEND_TYPE,OFFEND_RELIEVE_STATUS,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),LOAN_CODE_NOW,RULES_CODE,'1','0','BATCH',SYSDATE,'BATCH',SYSDATE
FROM JK.T_JK_ANTIFRAUD_OFFEND_SALES 
WHERE LOAN_CODE_NOW=loancode
GROUP BY LOAN_CODE_NOW,RULES_CODE;
GET DIAGNOSTICS T_COUNT = ROW_COUNT;
SUM_COUNT:=SUM_COUNT+T_COUNT;
...
INSERT INTO JK.T_JK_ANTIFRAUD_CASE(ID,LOAN_CODE_NOW,RULES_CODE,CASE_CODE,LOAN_CODE,CASE_HANDLE_DAY,CASE_HANDLE_BY,
                                LOAN_CUSTOMER_NAME,DICT_ANTIFRAUD_TYPE,CASE_RISK_MSG,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),A.LOAN_CODE,'B001',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
...
GROUP BY A.LOAN_CODE,C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,C.DICT_JUDGE_SECOND_CODE,
         C.DETAIL_JUDGE_RISK_MSG
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'B002',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
       C.DICT_JUDGE_SECOND_CODE,C.DETAIL_JUDGE_RISK_MSG,'BATCH',SYSDATE,'BATCH',SYSDATE 
...
FROM TEMP_CASE A INNER JOIN JK.T_JK_BACKLIST_ALL B ON B.DICT_MARK_TYPE='0' AND A.CONTACT_MOBILE=B.BLACK_MSG
                 INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE AND C.DICT_CASE_RESULT='0'
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'B006',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
       C.DICT_JUDGE_SECOND_CODE,C.DETAIL_JUDGE_RISK_MSG,'BATCH',SYSDATE,'BATCH',SYSDATE
...
GROUP BY A.LOAN_CODE,C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,C.DICT_JUDGE_SECOND_CODE,
         C.DETAIL_JUDGE_RISK_MSG
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'B007',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
       C.DICT_JUDGE_SECOND_CODE,C.DETAIL_JUDGE_RISK_MSG,'BATCH',SYSDATE,'BATCH',SYSDATE 
...
GROUP BY A.LOAN_CODE,C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,C.DICT_JUDGE_SECOND_CODE,
         C.DETAIL_JUDGE_RISK_MSG
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'B008',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
       C.DICT_JUDGE_SECOND_CODE,C.DETAIL_JUDGE_RISK_MSG,'BATCH',SYSDATE,'BATCH',SYSDATE  
...
GROUP BY A.LOAN_CODE,C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,C.DICT_JUDGE_SECOND_CODE,
         C.DETAIL_JUDGE_RISK_MSG
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'B009',C.JUDGE_CASE_CODE,B.LOAN_CODE,C.JUDGE_PROC_DATE,C.JUDGE_PROC_BY,C.LOAN_CUSTOMER_NAME,
       C.DICT_JUDGE_SECOND_CODE,C.DETAIL_JUDGE_RISK_MSG,'BATCH',SYSDATE,'BATCH',SYSDATE 
...
INSERT INTO JK.T_JK_ANTIFRAUD_BLACKLIST(ID,LOAN_CODE_NOW,RULES_CODE,LOAN_CODE,DICT_MARK_TYPE,DICT_BLACKLIST_TYPE,BLACKLIST_MSG,BLACKLIST_RISK_MSG,
                                     BLACKLIST_RELATION,ADD_BLACK_TIME,ADD_BLACK_TYPE,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME)
SELECT GETUUID32(),A.LOAN_CODE,'W001',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'本人('||CUSTOMER_NAME||')',
...
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON (A.PHONE_ONE!='' AND A.PHONE_ONE=B.BLACK_MSG) OR (A.PHONE_TWO!='' AND A.PHONE_TWO=B.BLACK_MSG) 
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE 
GROUP BY A.LOAN_CODE,B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,A.CUSTOMER_NAME,B.CREATE_TIME,B.DICT_SOURCE
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'W002',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'配偶('||CUSTOMER_NAME||')',
       B.CREATE_TIME,B.DICT_SOURCE,'BATCH',SYSDATE,'BATCH',SYSDATE 
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON A.MATE_TEL=B.BLACK_MSG
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE
GROUP BY A.LOAN_CODE,B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,A.CUSTOMER_NAME,B.CREATE_TIME,B.DICT_SOURCE
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,CASE WHEN RELATION_TYPE='0' THEN 'W003' WHEN RELATION_TYPE='1' THEN 'W004' WHEN RELATION_TYPE='2' THEN 'W005' END,
       B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,
...
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON A.CONTACT_MOBILE=B.BLACK_MSG
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE 
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'W006',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'本人('||CUSTOMER_NAME||')',
       B.CREATE_TIME,B.DICT_SOURCE,'BATCH',SYSDATE,'BATCH',SYSDATE 
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON A.COMP_NAME=B.BLACK_MSG
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE 
GROUP BY A.LOAN_CODE,B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,A.CUSTOMER_NAME,B.CREATE_TIME,B.DICT_SOURCE
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'W007',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'本人('||CUSTOMER_NAME||')',
       B.CREATE_TIME,B.DICT_SOURCE,'BATCH',SYSDATE,'BATCH',SYSDATE 
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON A.COMP_TEL=B.BLACK_MSG
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE 
GROUP BY A.LOAN_CODE,B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,A.CUSTOMER_NAME,B.CREATE_TIME,B.DICT_SOURCE
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'W008',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'本人('||CUSTOMER_NAME||')',
       B.CREATE_TIME,B.DICT_SOURCE,'BATCH',SYSDATE,'BATCH',SYSDATE 
FROM TEMP_BACKLIST A INNER JOIN JK.T_JK_BACKLIST_ALL B ON A.CUSTOMER_CERT_NUM=B.BLACK_MSG
                     INNER JOIN JK.T_JK_ANTIFRAUD_JUDGE C ON B.LOAN_CODE=C.LOAN_CODE 
GROUP BY A.LOAN_CODE,B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,A.CUSTOMER_NAME,B.CREATE_TIME,B.DICT_SOURCE
UNION ALL
SELECT GETUUID32(),A.LOAN_CODE,'W009',B.LOAN_CODE,B.DICT_MARK_TYPE,B.DICT_BLACK_TYPE,B.BLACK_MSG,C.JUDGE_RISK_MSG,'配偶('||CUSTOMER_NAME||')',
       B.CREATE_TIME,B.DICT_SOURCE,'BATCH',SYSDATE,'BATCH',SYSDATE 
...
/*****************************判定结果处理********************************/
SELECT INTO MSG STRING_AGG(LABEL,',') FROM (
		SELECT B.LABEL 
		FROM JK.T_JK_ANTIFRAUD_OFFEND_INFO A 
		LEFT JOIN T_GL_DICT B ON A.DICT_OFFEND_TYPE = B.VALUE AND B.TYPE='jk_offend_info' 
		WHERE LOAN_CODE = loancode GROUP BY B.LABEL); 
IF SUM_COUNT>0 THEN
	SELECT INTO LOANSTAUTS CASE WHEN DICT_LOAN_STATUS = '81' THEN '5' ELSE '6' END FROM JK.T_JK_LOAN_INFO WHERE LOAN_CODE=loancode LIMIT 1;
	BACKVALUE:='1';
	REPORT_ID:=GETUUID32();-- 提报反欺诈表
	INSERT INTO JK.T_JK_ANTIFRAUD_REPORT(ID,R_STATUS_HIS_ID,LOAN_CODE,DICT_REPORT_TYPE,MENTION_GROUP_ID,ANTIFRAUD_MSG,
				     MENTION_USER_ID,DICT_CHECK_TYPE,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME) 		   
	VALUES (REPORT_ID,HIS_ID,loancode,'3','3',MSG,'SYSTEM','3','SYSTEM',SYSDATE,'SYSTEM',SYSDATE); 
	JUDGE_ID:=GETUUID32();-- 反欺诈判定表
	INSERT INTO T_JK_ANTIFRAUD_JUDGE(ID,R_REPORT_ID,LOAN_CODE,DICT_INTO_SOURCE,DICT_CASE_RESULT,CREATE_TIME,CREATE_BY,MODIFY_TIME,MODIFY_BY) 
	VALUES (JUDGE_ID,REPORT_ID,loancode,'3','4',SYSDATE,'SYSTEM',SYSDATE,'SYSTEM'); 
	JUDGE_RESULT := '触发';
ELSE
	SELECT INTO LOANSTAUTS CASE WHEN DICT_LOAN_STATUS <> '81' THEN '14' ELSE DICT_LOAN_STATUS END FROM JK.T_JK_LOAN_INFO WHERE LOAN_CODE=loancode LIMIT 1;  
END IF;  
HIS_ID:=GETUUID32();--历史记录表
INSERT INTO JK.T_JK_LOAN_STATUS_HIS(ID,LOAN_CODE,DICT_LOAN_STATUS,OPERATE_STEP,DICT_SYS_FLAG,OPERATE_RESULT,OPERATOR,
				                    OPERATE_ROLE_ID,ORG_CODE,OPERATE_TIME,CREATE_BY,CREATE_TIME,MODIFY_BY,MODIFY_TIME) 
VALUES (HIS_ID,loancode,LOANSTAUTS,'反欺诈引擎','MODULE_APPROVE',JUDGE_RESULT,'系统','SYSTEM','SYSTEM',SYSDATE,'SYSTEM',SYSDATE,'SYSTEM',SYSDATE);
--更新借款信息表中的状态
UPDATE JK.T_JK_LOAN_INFO SET DICT_LOAN_STATUS=LOANSTAUTS WHERE LOAN_CODE=loancode;
--返回判定结果
outvalue:=BACKVALUE;
END;
$BODY$
